<?php
/**
 * GitHub Update System for EWEB Elementor Bridge
 * Allows the plugin to receive updates directly from a GitHub repository.
 * Ported from EWEB Starter Helper.
 */
...
class EWEB_Bridge_Updater {
	...
	public function plugin_popup( $result, $action, $args ) {
		if ( $action !== 'plugin_information' || ! isset( $args->slug ) || $args->slug !== 'eweb-elementor-bridge' ) {
			return $result;
		}
		...
		$res = new stdClass();
		$res->name = 'EWEB Elementor Bridge';
		$res->slug = 'eweb-elementor-bridge';
		$res->version = $remote->tag_name;
		$res->author = 'Yisus Dev';
		$res->homepage = 'https://github.com/' . $this->github_user . '/' . $this->github_repo;
		$res->download_link = $remote->zipball_url;
		$res->sections = [
			'description' => 'Motor de integración industrial para Elementor (V10+).',
			'changelog'   => isset($remote->body) ? $remote->body : 'No changelog available.',
		];
